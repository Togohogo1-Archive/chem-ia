...
If the order of the reaction is zero, the rate of reaction will not be dependent on the concentration of the reactants, that is $V = k[A]^0 = k$. If the order of the reaction is one, the rate of reaction will be proportional to the concentration of the reactants, that is $V = k[A]^1 = k[A]$.

\subsection{Michaelis Menten Enzyme Kinetics}
When reactions are enzyme-catalyzed, enzyme kinetics may also be used to predict the behvaiour of the reaction. The Michaelis Menten model, proposed by Leonor Michaelis and Maud Menten in 1913, is concerned with the relationship between initial reaction velocity $V_0$ (rate of enzyme catalysis), and the substrate concentration (Berg et al., 2006):
\begin{equation}
...
\begin{itemize}[topsep=\parskip, noitemsep]
    ...
    \item ES denotes the enzyme substrate complex, an intermediate step in which the enzyme temporarily binds to the substrate, which eventually gets dissociated into the same enzyme and the product
\end{itemize}
...
\begin{tabularx}{\textwidth} {
    ...
    \hline
    Risk of hotplate burns & It is important to not touch the hotplate, even when it is off. Once finished using, the hotplate and any lab apparatus that came in contact with it should be cooled before storage.\\
    \hline
    ...
    \hline
    Dumping substances down the drain & Yeast should not be dumped down the drain unless it is dissolved and heavily diluted (Holmes, n.d.). Similarly, dumping concentrated ethanol is hazardous unless heavily diluted as well (“Sewer Disposal: What Can Go Down the Drain?”, 2021). \\
    \hline
...
\begin{enumerate}[topsep=\parskip, noitemsep]
    ...
    \item The water was heated until it reached approximately 37\,\si{\celsius}. The temperature of the hotplate was kept at in between 40\,\si{\celsius} and 46\,\si{\celsius} to account for cooling.
    ...
    \item Magnetic stirring was activated for about 10 minutes to mix the yeast solution. During this phase, observations were recorded and the \ce{CO2} gas sensor was warmed up to read a baseline \ce{CO2} concentration.
    ...
    \item The \ce{CO2} gas sensor was wiped to remove condensation and fanned to blow away residual \ce{CO2}. The bottle was thoroughly rinsed and also fanned.
...
\begin{tabularx}{\textwidth} {
    ...
    | >{\hsize=.4\hsize \linewidth=\hsize \raggedright\arraybackslash}X |}
    \hline
    \textbf{Mass /\ce{g} $\pm0.01$} & \textbf{Concentration /\si{M}} & \textbf{Observations} \\
    \hline
...
Additionally, the two graphs below displays all the raw data with the line of best fits ommited to avoid clutter. A full list of raw data in tabular form can be found in the appendix:
\begin{figure}[H]
    \renewcommand{\figurename}{Graph}
    \setcounter{figure}{2}
    \centering
...
\begin{tabularx}{\textwidth} {
    ...
    \hline
    $0.0056 \pm 0.0006$ & $y=35.324x, R^2=0.9962$ & $y=81.960x, R^2=0.9936$ & $59 \pm 20$ \\
    \hline
...
With the processed data in Table (5), reaction velocity can be plotted against glucose concentration to obtain the Michaelis Menten model. Doing a curve fit in excel reveals $V_{max} \approx 64.1\,\si{ppm.s^{-1}}$ and $K_m \approx 0.0001\,\si{M}$
\begin{figure}[H]
    \renewcommand{\figurename}{Graph}
    \setcounter{figure}{4}
    \centering
...
For the most part, all concentration-time graphs followed a linear trend for the first 10 minutes of the reaction. This implies a zero order reaction, meaning at this stage, the initial reaction velocity is independent of the glucose concentration. However, none of the graphs exhibited the end behaviour if the reaction approached equilibrium, because the reaction was stopped after 10 minutes. Thus, the fermentation process of yeast was a slower reaction than proposed in the hypothesis.

\medskip

The processed data in Graph (5) somewhat fits the general trend of the Michaelis Menten model. The theoretical curve fitted to the data is valid because it crosses through all the vertical error bars. From the calculated values of $V_{max}$ and $K_m$, several conclusions can be made about the fermentation process of yeast in the experiment. A very low value of $K_m$ suggests that enzymes have a very high affinity for substrate binding (Berg et al., 2006), which makes sense because according to Equation (15), a small $K_m$ implies a large denominator, meaning the concentration of enzyme substrate complex is significantly larger.

\medskip

Additionally, the Michaelis Menten model is consistent with the raw data. From around 0.005\,\ce{M} and upwards, the initial reaction velocity starts to become independent of the substrate concentration. This is consistent with the concentration-time graphs because they start off as zero order reactions, and their initial reaction velocity seems to reside in the range where the Michaelis Menten curve starts to become asymptotic.

\section{Evaluation}
The largest potential source of error was deduced to be the temperature of the water, because it was difficult to keep constant, especially in between trials when the warm water bath was refilled. The temperature reading on the magnetic stirring hotplate wasn't the same as the water in an attempt to account for cooling, This means that it was adjusted a few times, leaving room for a range of solution temperatures, affecting the rate of reaction of carbon dioxide production.

\medskip

Observing the Michaelis Menten plot, the reaction velocity should increase as substrate concentration increases. However, it decreases with substrate concentration from 0.25\,\si{g} to 1.00\,\si{g} of glucose added. This implies that the reaction velocity should have been higher. The two major reasons assumed for this major discrepancy are listed as follows.

\medskip

First of all, adding yeast and glucose to the bottle when the insides are wet causes some of the substances to stick to the sides. Although the bottle was swirled with water after this happened, it's likely all not all the yeast and glucose ended up reacting.

\medskip

Secondly, the time between experiments could have been too short. Usually the first experiment of the day yields the most accurate results, but it's possible that not enough time was given for the carbon dioxide in the \ce{CO2} gas sensor to completely diffuse out, decreasing the effectiveness in the instrument.
...
\medskip
These skewed results may also reflect the raw data obtained from the plotted concentration-time graphs. For example, the reaction velocity for 0.25\,\si{g} of glucose added in Graph (3) seems to change at about 350 seconds. This sudden change in slope is believed to originate from incoherent stirring, since the dent at the bottom of the Nalgene bottle makes it hard for the stir bar to reach every corner. Lastly, for 1.00\,\si{g} of glucose added in Graphs (3) and (4), the plots seem to vaguely emulate the behavior of a first order reaction. A potential explanation for this could be the time it takes for a larger amount of glucose to dissolve, giving an underestimate for the initial reaction velocity. Unsurprisingly, these three graphs also have the smallest $R^2$ values with regards to their lines of best fit.
